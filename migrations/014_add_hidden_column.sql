-- Add hidden flag to contacts so non-distributor contacts (manufacturers, etc.)
-- can be removed from all active views without deleting data.

-- ============================================================
-- 1. Column + index
-- ============================================================
ALTER TABLE contacts ADD COLUMN IF NOT EXISTS hidden BOOLEAN DEFAULT FALSE;
CREATE INDEX IF NOT EXISTS idx_contacts_hidden ON contacts(hidden);

-- ============================================================
-- 2. Backfill: hide all non-distributor scored contacts
-- ============================================================
UPDATE contacts SET hidden = TRUE
WHERE company_type IS DISTINCT FROM 'distributor'
  AND company_type IS NOT NULL;

-- ============================================================
-- 3. Update claim_next_contact to exclude hidden contacts
-- ============================================================
