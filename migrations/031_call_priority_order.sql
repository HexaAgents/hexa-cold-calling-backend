-- Reprioritize the call tracker claim order.
--
-- Identical to migration 029 except for ORDER BY. The queue is now sorted by:
--   1. Least called first (times_called ascending) — fresh contacts before
--      retries; the old "due didnt_pick_up retries jump the queue" rule is
--      intentionally removed.
--   2. Later local time of day first (seconds since local midnight,
--      descending). Contacts with an unknown timezone rank after all
--      known-timezone contacts within the same called-count group.
--   3. Higher score first.
--   4. created_at ascending as a stable tiebreak.
--
-- The WHERE clause (eligibility, location filters, business hours) is
...
BEGIN
  ...
  ORDER BY
    COALESCE(c.times_called, 0) ASC,
    EXTRACT(EPOCH FROM (NOW() AT TIME ZONE c.timezone)::time) DESC NULLS LAST,
