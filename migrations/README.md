...
- `033_todo_time_estimates.sql`: AI time estimates for todos — `estimated_hours_min/max` (the LLM's hour range, written once by a background task), `estimate_status` (pending → done/failed, with failed terminal so each task costs at most one OpenAI call), and `actual_hours` (reported when ticking a task off; feeds the estimator's calibration examples).
